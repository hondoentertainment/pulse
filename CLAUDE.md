...
> **App-mode note:** the shipped default is the **venue** discovery app
> (`VITE_APP_MODE` unset or `venue`); the legacy Pulse Signal daily-check-in
> shell runs under `VITE_APP_MODE=signal`. See `src/lib/app-mode.ts` and
> `ARCHITECTURE.md`. Some auth-flow wording below predates that flip and
> describes the Signal shell.
...
- Offline writes are queued by `src/lib/offline-queue.ts`; `usePulseSync` drains the queue to Supabase on startup and reconnect. The service worker also uses Workbox `backgroundSync` for `POST /rest/v1/pulses` (see `vite.config.ts`).
...
`SupabaseAuthProvider` (`src/hooks/use-supabase-auth.tsx`) gates the app. `App.tsx` shows `LoginScreen` when there's no session, `OnboardingFlow` when `hasCompletedOnboarding === false`, and the app shell otherwise. `VITE_E2E_AUTH_BYPASS=true` or `VITE_VISUAL_PREVIEW=true` short-circuits the gate (used by Playwright). Supabase enforcement isn't yet across all writes — assume any write you add still needs server-side guards.
