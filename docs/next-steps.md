...
| **8 PWA / offline** | Reliability (`6:92`) | `InstallAffordance`, `OfflineBanner`, `MapHomeSkeleton` | Home-screen install on the map tab (plus Settings). Offline banner + draft + skeleton bars; Keep browsing. |
| **9 Trust v2** | Trust at a glance (`6:2`) | `TrustPinChips` on map hover + Tonight + Surging | Freshness / Verified / density chips beyond the trust strip. |
