...
- **Functionality**: Real-time backend system that ingests posts from X/Twitter for configurable hashtags, converts them into a rolling Social Pulse score, correlates that activity with venue Pulse scores, and surfaces insights through an admin dashboard. Tracks hashtags, calculates social media engagement metrics, detects correlation patterns, and identifies when social buzz is driving venue energy.
- **Purpose**: Provide venue operators and admins with intelligence on how social media activity relates to real-world venue energy, enabling data-driven marketing decisions and identifying venues gaining social momentum before they peak.
- **Trigger**: Admin opens Social Pulse Dashboard from Settings → Dashboard automatically polls X API for tracked hashtags → Correlations calculated every 10 minutes → Real-time graphs update as new social posts arrive
- **Progression**: Admin navigates to Settings → Taps "Open Admin Dashboard" → Dashboard loads with overview → Admin configures tracked hashtags → Maps hashtags to specific venues (optional) → System polls X API every 60 seconds → Social posts ingested and deduplicated → Posts converted into 5min/15min/60min rolling windows → Social Pulse score calculated (0-100 based on volume, engagement, velocity) → Venue Pulse windows created in parallel → Pearson correlation calculated for 60min and 120min windows → Lag detection identifies if social leads or follows venue activity → Dashboard displays: Social Pulse graph, Venue vs Social overlay chart, Correlation insights with strength indicators, Social Buzz badges for venues with strong correlation → Admin filters by hashtag or venue → Insights update in real-time
- **Success criteria**: Social posts are ingested automatically with retry/backoff handling, deduplication prevents duplicate posts, rolling windows calculate correctly for all time ranges, correlation coefficient accurately measures relationship strength, lag detection identifies timing relationships, dashboard updates without manual refresh, venues with correlation ≥0.6 and score ≥40 receive "Social Buzz" badge, admin can add/remove hashtags, admin can map hashtags to venues, rate limits are respected with clear status indicators
